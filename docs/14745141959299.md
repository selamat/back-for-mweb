...
## Use DNS in kubernetes

#### Create kube-dns deployment

Replace `${DNS_DOMAIN}` with **cluster.local** and `${DNS_REPLICAS}` with 1 or 2.
Make `kube_master_url` set properly.
  ...
  template:
    ...
    spec:
      ...
      - name: kubedns
        ...
        args:
        # command = "/kube-dns"
        - --kube_master_url=http://${kube-apiserver-addr}:${kube-apiserver-port}
        - --domain=${DNS_DOMAIN}.
        - --dns-port=10053
        ports:
        - containerPort: 10053
...
Use `kubelet create -f $fileName` to start dns pod. Check logs.

#### Create kube-dns service

Replace `${DNS_SERVER_IP}` with a specify ip and this ip must belongs to `--service-cluster-ip-range` in your kube-apiserver. Then create dns-service.
...
Add the following parameters to kubelet start command
...
#### Check if dns service works well
...
# Or get into pods, use ping $serviceIP to check.
```
